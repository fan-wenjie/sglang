...
class TestPositionsSurviveTheWire(CustomTestCase):
    def test_a_plain_sequence_comes_back_as_one(self):
        ...
        self.assertTrue(
            torch.equal(unpack_positions(pack_positions(positions)), positions)
        )
    ...
    def test_the_length_matches_the_tokens_it_belongs_to(self):
        ...
        for tokens in (1, 5, 128):
            ...
            self.assertEqual(
                pack_positions(torch.arange(3 * tokens).reshape(3, tokens)).shape[1],
                tokens,
            )
    ...
    def test_it_is_integral_on_the_wire(self):
        ...
        self.assertEqual(
            pack_positions(torch.arange(4).to(torch.int32)).dtype, torch.int64
        )
