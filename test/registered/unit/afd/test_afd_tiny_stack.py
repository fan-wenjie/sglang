...
NOT registered for CI: this file builds a real Qwen3.5 stack, which needs a CUDA device
(sglang's RMSNorm dispatches to sgl_kernel::gemma_rmsnorm, which has no CPU kernel). It runs
by hand on a card. Registering it in the CPU suite would make it fail for the environment
rather than for the code, which is the kind of red that gets a file deleted.

Unregistered is not the same as uncollected: `pytest test/registered/unit/` is the command the
contribution guide gives, and on a machine with no card this file used to answer it with eight
errors from `init_process_group`. The skip below makes that answer "skipped" instead, which is
what it means -- the same distinction the registration note is about, one level down.
...
@unittest.skipUnless(
    torch.cuda.is_available(), "builds the model's own layers, which have no CPU kernel"
)
class TestTheFixtureIsTheModelsOwnClasses(CustomTestCase):
    ...
    def test_the_projections_live_where_the_span_reaches_for_them(self):
        ...
        self.assertNotIsInstance(
            getattr(softmax, "self_attention", None), torch.nn.Module
        )
    ...
    def test_the_convolution_is_depthwise_and_its_weight_is_taps_last(self):
        ...
        self.assertEqual(
            weight.shape[-1], TINY["linear_conv_kernel_dim"], "taps are last"
        )
    ...
    def test_no_parameter_is_still_zero(self):
        ...
        zero = [
            n
            for n, p in self.stack.named_parameters()
            if float(p.float().abs().max()) == 0.0
        ]
