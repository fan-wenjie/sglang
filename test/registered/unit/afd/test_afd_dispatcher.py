...
from sglang.srt.afd.protocol import OP_FFN, OP_LAYER, Frame
...
class TestTheMergeIsRowsForRows(CustomTestCase):
    ...
    def test_a_layer_never_rides_with_another_layer(self):
        ...
        self.assertEqual(
            upstream.calls, [], "two different layers completed a bus between them"
        )
...
class TestOnlyIndependentRowsAreMerged(CustomTestCase):
    def test_an_op_that_calls_back_is_relayed_one_for_one(self):
        ...
        upstream = Upstream()
        stop = Stop(upstream, min_batch=2, max_wait_s=10.0)
        stop.offer(a_frame(1, 0, rows=1, value=1.0, op=OP_LAYER), Sink())
        stop.offer(a_frame(2, 0, rows=1, value=2.0, op=OP_LAYER), Sink())
        self.assertEqual(
            upstream.calls, [("relay", 0, OP_LAYER), ("relay", 0, OP_LAYER)]
        )
