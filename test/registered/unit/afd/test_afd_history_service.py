        ...
        with self.assertRaises(RuntimeError) as caught:
            service(
                Frame(
                    1,
                    0,
                    (torch.randn(1, VH * D), torch.randn(1, VH * D), torch.rand(1, VH)),
                    OP_STATE_UPDATE,
                )
            )
...
class TestAScanAdvancesTheStateBetweenAChunksTokens(CustomTestCase):
    ...
    def a_chunk(self, n=4, seed=5):
        ...
        return (
            ...
            r(n, VH * D),
            r(n, VH * D),
            torch.rand(n, VH, generator=g) * 0.5 + 0.5,
            torch.rand(n, VH, generator=g),
        )

    def test_it_equals_the_same_tokens_read_one_at_a_time(self):
        from sglang.srt.afd.linear_history import (  # noqa: F401 -- shape parity only
            gates,
        )
        ...
        for t in range(n):
            ...
            one_at_a_time(
                Frame(
                    1,
                    0,
                    (k[t : t + 1], v[t : t + 1], alpha[t : t + 1], beta[t : t + 1]),
                    OP_STATE_UPDATE,
                )
            )
        ...
        torch.testing.assert_close(
            cache.state[0, slot],
            other.state[0, other.slot_of(11)],
            rtol=1e-4,
            atol=1e-5,
        )
    ...
    def test_two_requests_in_one_chunk_each_advance_their_own_slot(self):
        """A prefill batch holds every request admitted that step, not one.

        This is the bug that killed the deployment: the scan refused any chunk whose rows did not
        all share one slot, so the arrangement served concurrency 1 and took the host down with
        `PoolClosed` at concurrency 2 -- the refusal was raised inside the pool client's receiver
        thread. The rows arrive grouped by request, and the per-row loop already advances each
        against its own slot, so the answer must equal serving the two requests separately.
        """
        n = 3
        both, service = a_service(ids=(11,) * n + (22,) * n)
        q, k, v, alpha, beta = self.a_chunk(2 * n)
        got = service(Frame(1, 0, (q, k, v, alpha, beta), OP_STATE_SCAN))[0]

        # the same rows, one request at a time, which is what a chunk of two has to equal
        wanted = []
        for who, lo in ((11, 0), (22, n)):
            alone, apart = a_service(ids=(who,) * n)
            rows = slice(lo, lo + n)
            wanted.append(
                apart(
                    Frame(
                        1,
                        0,
                        (q[rows], k[rows], v[rows], alpha[rows], beta[rows]),
                        OP_STATE_SCAN,
                    )
                )[0]
            )
            torch.testing.assert_close(
                both.state[0, both.slot_of(who)],
                alone.state[0, alone.slot_of(who)],
                rtol=1e-4,
                atol=1e-5,
            )
        torch.testing.assert_close(got, torch.cat(wanted, dim=0), rtol=1e-4, atol=1e-5)

    def test_a_request_split_around_another_is_still_refused(self):
        """The case the old guard was really for, and the only one left that must raise.

        Rows grouped by request are safe; rows INTERLEAVED are not. The per-row loop would scan
        slot 11's second run straight on from its first, as though request 22's tokens in between
        had never happened -- a history threaded through positions it never saw, with nothing in
        the output to say so.
        """
        cache, service = a_service(ids=(11, 22, 11))
        q, k, v, alpha, beta = self.a_chunk(3)
        with self.assertRaises(RuntimeError) as caught:
