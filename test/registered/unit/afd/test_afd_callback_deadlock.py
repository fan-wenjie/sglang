...
from sglang.srt.afd import (  # noqa: F401  -- claims the span ops in the departure table
    pool_span,
)
...
class Runner:
    ...
    def run_prologue(
        self, request_ids, embedded, positions, on_query=None, windows=None
    ):
    ...
    # the server asks `runner.states.release` -- the object that would hold a slot; these
    # stubs hold nothing, so the runner IS its own state holder
    states = property(lambda self: self)
    ...
    def run(
        self, request_ids, group, attn_output, positions, on_query=None, windows=None
    ):
...
class Pool:
    ...
    def __init__(self, asks=3, min_batch=1):
        ...
        self.runner.model = types.SimpleNamespace(
            model=types.SimpleNamespace(
                layers=[types.SimpleNamespace(linear_attn=object()) for _ in range(4)]
            )
        )
        self.departure.runner = self.runner
...
@unittest.skipUnless(
    ...
    "the span handover records a CUDA event, so this needs a device",
)
        ...
        def call():
            try:
                handle = client.issue_frame(
                    1,
                    0,
                    (torch.zeros(1, 4), torch.zeros(1, 1), torch.zeros(1, 1)),
                    OP_SPAN,
                )
                result["reply"] = client.collect_frame(
                    handle._replace(op=OP_SPAN), "cpu"
                )
...
class TestAReleaseReachesTheRecurrentState(CustomTestCase):
    ...
    def test_the_pool_forgets_the_request_it_is_told_to(self):
        ...
        try:
            ...
            # the pool releases within the CALLER's namespace: the high bits are the
            # socket's, the low 40 are the id the host sent
            self.assertEqual([r & ((1 << 40) - 1) for r in pool.runner.released], [7])
            self.assertEqual(
                float(reply[0][0, 0]), 1.0, "the reply says how many slots went"
            )
        finally:
            pool.close()
...
class TestTheReplyCarriesTheOpItWasAskedWith(CustomTestCase):
    ...
    def test_a_span_with_windows_is_taken_and_they_reach_the_runner(self):
        """The fourth tensor -- the convolution windows -- must pass the departure's door.

        A refusal used to stand there, left over from the ring-lending ledger, and the first
        live flight of the cooked protocol died on it: every unit test entered at the runner,
        none at the door. This one knocks with four tensors.
        """
        pool = Pool(asks=1)
        client = self.a_client(pool)
        done, got = threading.Event(), {}
        windows = torch.full((1, 6), 7.0)
        ...
        def call():
            try:
                handle = client.issue_frame(
                    1,
                    0,
                    (torch.zeros(1, 4), torch.zeros(1, 1), torch.zeros(1, 1), windows),
                    OP_SPAN,
                )
                client.collect_frame(handle._replace(op=OP_SPAN_Q), "cpu")
                got["kv"] = client.collect_frame(handle, "cpu")
            except BaseException as e:  # noqa: BLE001
                got["error"] = e
            finally:
                done.set()
        ...
        threading.Thread(target=call, daemon=True).start()
        try:
            self.assertTrue(done.wait(DEADLINE), "the windowed span was never answered")
            self.assertIsNone(got.get("error"), f"{got.get('error')}")
            self.assertEqual(len(got["kv"]), 2)
            torch.testing.assert_close(pool.runner.windows_seen.cpu(), windows)
        finally:
            pool.close()
...
class ScanRunner(Runner):
    ...
    def linear_attention(self, attn, request_ids, layer, hidden):
    ...
    def run(
        ...
        for layer in range(self.asks):
            self.seen.append(
                tuple(self._local.ask_host(layer, request_ids, q, step=step).shape)
            )
        if on_query is not None:
        ...
        def call():
            try:
                handle = client.issue_frame(
                    1,
                    0,
                    (torch.zeros(3, 4), torch.zeros(3, 1), torch.zeros(3, 1)),
                    OP_SPAN,
                )
                result["reply"] = client.collect_frame(
                    handle._replace(op=OP_SPAN), "cpu"
