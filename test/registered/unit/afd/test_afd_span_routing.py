...
class Recorder:
    ...
    def issue_frame(self, request_id, layer, tensors, op):
        self.issued.append(
            {"request_id": request_id, "layer": layer, "op": op, "tensors": tensors}
        )
    ...
    def setUp(self):
        ...
        for span in self.routing.spans:
            for layer_id in span[1:]:
                self.routing._make_pass_through(
                    self.stack.model.layers[layer_id], layer_id
                )
...
class TestTheTwoHalvesAreTakenUnderDifferentKeys(CustomTestCase):
    ...
    def test_the_read_point_is_collected_under_its_own_opcode(self):
        client = a_client()
        handle = client.issue(
            3, torch.zeros(1, 4), torch.tensor([0]), torch.tensor([5])
        )
...
class TestTheRowIdsTravel(CustomTestCase):
    ...
    def test_mrope_positions_keep_their_axes(self):
        client = a_client()
        client.issue(
            3, torch.zeros(2, 8), torch.tensor([5, 9]), torch.arange(6).reshape(3, 2)
        )
...
class TestTheOpcodesNameTheThreeShapes(CustomTestCase):
    ...
    def test_the_opcode_travels_with_the_frame(self):
        client = a_client()
        client.issue(
            0, torch.zeros(1, 4), torch.tensor([0]), torch.tensor([0]), OP_SPAN_ENTER
        )
        self.assertEqual(client.client.issued[0]["op"], OP_SPAN_ENTER)


def a_runner():
    """Any object standing in for the span runner has to carry the bit the pool advertises.

    A bare `object()` was here, and it stopped being enough when the pool started reading the
    HELLO word off the runner instead of hardcoding 8 -- which is the whole point: the thing that
    serves whole spans is what says so. The symptom was the host refusing to start against a pool
    that did serve them, at connect time, saying the two were configured for different cuts.
    """
    return types.SimpleNamespace(capability_bit=8)
...
class TestOneDepartureServesOneJob(CustomTestCase):
    ...
    def test_a_mixed_queue_is_refused(self):
        ...
        riding = [
            (Frame.one(1, 3, torch.zeros(1, 4), OP_SPAN), None),
