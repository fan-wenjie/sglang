...
class TestAReservedOpIsRefusedRatherThanQueued(CustomTestCase):
    ...
    def test_it_is_refused_on_a_pool_with_nothing_attached(self):
        """The placement, not the message. A pool with no attention and no extra runner takes
        ...
        departure = self._departure()
        self.assertIsNone(departure.runner)
        with self.assertRaises(ConnectionError):
            departure.answer_directly(
                Frame(1, 0, (torch.zeros(1, 4),), OP_LINEAR), None
            )
    ...
    def test_an_op_this_pool_does_serve_still_falls_through_to_the_queue(self):
        ...
        departure = self._departure()
        self.assertFalse(
            departure.answer_directly(Frame(1, 0, (torch.zeros(1, 4),), OP_FFN), None)
        )
