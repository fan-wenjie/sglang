...
class TestSplitIsTheSameAttention(CustomTestCase):
    def test_exact_in_float64_at_this_model_shapes(self):
        ...
        self.assertLess(
            worst, 1e-12, f"the split is not the same attention: {worst:.2e}"
        )
    ...
    def test_the_sweep_never_sees_the_current_value(self):
        ...
        self.assertTrue(
            torch.equal(a[0], b[0]), "the sweep moved when only v_j changed"
        )
    ...
    def test_any_cut_in_any_order_gives_one_sweep(self):
        ...
        for cuts in ([0, 1, 399], [0, 200, 399], [0, 100, 250, 399], [0, 399]):
            ...
            for order in itertools.permutations(range(len(parts))):
                ...
                worst = max(
                    worst,
                    float((o - whole[0]).abs().max()),
                    float((lse - whole[1]).abs().max()),
                )
        self.assertLess(
            worst, 1e-12, f"a split cache disagreed with one sweep by {worst:.2e}"
        )

    def test_a_boundary_off_by_one_is_not_a_rounding(self):
        """What the runtime verify number has to be read against.

        A runtime check reports how far the partitioned attention sits from the fused one on real
        traffic -- on the derived line, which is where the partition is consulted; this branch
        carries the partition without a caller. Either way a small number only means something if
        a WRONG partition would have given a large one. The realistic mistake is at the boundary: the sweep is meant to cover
        ...
        short = float(
            (merged(cached - 1) - reference).abs().max() / reference.abs().max()
        )
        print(f"    honest {honest:.2e}   one position short {short:.2e}")
        self.assertLess(
            honest, 1e-12, "a partition of the cache reproduces the whole of it"
        )
        self.assertGreater(
            short,
            1e-2,
