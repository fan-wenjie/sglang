...
SEQ_BASE = D.ACTIVATION_MIN_TOKENS + 100
...
class TestDecodeTimeBlockClose(CustomTestCase):
    ...
    def test_no_close_before_a_full_block(self):
        be = self._backend(prefill=D.ACTIVATION_MIN_TOKENS)
        be._maybe_close_blocks(
            self._fb(D.ACTIVATION_MIN_TOKENS + D.CLOSE_BLOCK - 1), [0]
        )
        self.assertEqual(
            be._close_state[(0, self.LID2)]["closed"], D.ACTIVATION_MIN_TOKENS
        )
    ...
    def test_crossing_the_threshold_catches_up_all_blocks(self):
        ...
        be = self._backend(prefill=8192)
        be._maybe_close_blocks(self._fb(D.ACTIVATION_MIN_TOKENS + 7), [0])
        self.assertEqual(
            be._close_state[(0, self.LID2)]["closed"], D.ACTIVATION_MIN_TOKENS
        )
    ...
    def test_close_advances_and_rewrites_kept(self):
        be = self._backend(prefill=D.ACTIVATION_MIN_TOKENS)
        seq = D.ACTIVATION_MIN_TOKENS + D.CLOSE_BLOCK + 7
        be._maybe_close_blocks(self._fb(seq), [0])
        cl = be._close_state[(0, self.LID2)]
        self.assertEqual(cl["closed"], D.ACTIVATION_MIN_TOKENS + D.CLOSE_BLOCK)
        self.assertEqual(cl["sigma"].shape[0], D.ACTIVATION_MIN_TOKENS + D.CLOSE_BLOCK)
    ...
    def test_multiple_blocks_close_in_one_step(self):
        be = self._backend(prefill=D.ACTIVATION_MIN_TOKENS)
        seq = D.ACTIVATION_MIN_TOKENS + 3 * D.CLOSE_BLOCK + 1
        be._maybe_close_blocks(self._fb(seq), [0])
        self.assertEqual(
            be._close_state[(0, self.LID2)]["closed"],
            D.ACTIVATION_MIN_TOKENS + 3 * D.CLOSE_BLOCK,
        )
    ...
    def test_close_refreshes_a_built_tier(self):
        ...
        seq = D.ACTIVATION_MIN_TOKENS + D.CLOSE_BLOCK
        be._maybe_close_blocks(self._fb(seq), [0])
        # first close: caches empty -> extend covers ALL closed rows
        self.assertEqual(calls[0], ("extend", D.ACTIVATION_MIN_TOKENS + D.CLOSE_BLOCK))
