...
def _fill_with_values(stack, seed: int = 20260821) -> None:
    ...
    still_zero = [n for n, p in stack.named_parameters() if float(p.float().abs().max()) == 0.0]
    if still_zero:
        raise RuntimeError(
...
def build_tiny_stack(*, device: str, port: int = 29591):
    ...
    server_args = ServerArgs(
        ...
        attention_backend="torch_native", device=device,
    )
    ...
    kinds = ["full_attention" if hasattr(l, "attn") else "linear_attention" for l in stack.layers]
