...
MEASURED = dict(
    afd_mode="host", disable_cuda_graph=True, tp_size=1, pp_size=1,
    enable_dp_attention=False, speculative_algorithm=None, enable_lora=False,
    lora_paths=None, disable_overlap_schedule=True, enable_multimodal=False,
    rl_on_policy_target=None, cuda_graph_config=None,
)
...
class TestBrokenCannotBeOverridden(CustomTestCase):
    ...
    def setUp(self):
        self._saved = os.environ.get("AFD_ALLOW_UNTESTED")

    def tearDown(self):
        if self._saved is None:
            os.environ.pop("AFD_ALLOW_UNTESTED", None)
        else:
            os.environ["AFD_ALLOW_UNTESTED"] = self._saved

    def test_a_broken_feature_is_refused_even_when_named(self):
        os.environ["AFD_ALLOW_UNTESTED"] = "cuda_graph,lora"
    ...
    def test_an_untested_feature_runs_when_named(self):
        os.environ["AFD_ALLOW_UNTESTED"] = "tp_size"
