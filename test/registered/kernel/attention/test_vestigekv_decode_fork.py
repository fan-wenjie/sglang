...
def _run(q, kb, vb, indptr, indices, vestigekv, tiers, fence=True, affine=False):
    import msgspec
    ...
    decode_grouped_att_m_fwd(
        ...
        indptr,
        indices,
        msgspec.structs.replace(vestigekv, tiers=tiers, fence=fence, affine=affine),
        splits,
        SPLITS,
        1.0 / (LK**0.5),
        0.0,
...
class TestDecodeForkRowSource(CustomTestCase):
    def _case(self, fenced):
        ...
        vestigekv = _rows(gen)
        slot = int(vestigekv.slots[0])
        if fenced:
            vestigekv.fetch_ovf[slot] = 1
            n = int(vestigekv.seq[0])
            ids = vestigekv.r2t[slot, :n].clone()
            ids[-1] = int(vestigekv.loc[0])
        else:
            nk, nf = int(vestigekv.kept_len[slot]), int(vestigekv.fetch_len[slot])
            ids = torch.cat(
                [vestigekv.kept_buf[slot, :nk], vestigekv.fetch_buf[slot, :nf]]
            )
        indices = ids.to(torch.int64)
        indptr = torch.tensor([0, indices.numel()], dtype=torch.int32, device="cuda")
        a_out, a_lse = _run(
            q, pool, pool[:, :, :LV], indptr, indices, vestigekv, tiers=False
        )
        b_out, b_lse = _run(
            q, pool, pool[:, :, :LV], indptr, indices, vestigekv, tiers=True
        )
    ...
    def test_affine_equals_the_page_table_when_it_is_contiguous(self):
        ...
        n = int(vestigekv.seq[0])
        base = 7
        vestigekv.r2t[slot, :n] = base + torch.arange(
            n, dtype=torch.int32, device="cuda"
        )
        vestigekv.loc.fill_(base + n - 1)  # the affine arm has no last-row fixup
        indices = vestigekv.r2t[slot, :n].to(torch.int64)
    ...
    def test_fence_off_ignores_the_overflow_flag(self):
        ...
        vestigekv = _rows(gen)
        slot = int(vestigekv.slots[0])
        vestigekv.fetch_ovf[slot] = 1  # the scan overflowed; the fallback is off
        nk, nf = int(vestigekv.kept_len[slot]), int(vestigekv.fetch_len[slot])
        indices = torch.cat(
            [vestigekv.kept_buf[slot, :nk], vestigekv.fetch_buf[slot, :nf]]
        ).to(torch.int64)
...
class TestDecodeForkRegisters(CustomTestCase):
    ...
    def test_the_fence_costs_no_registers(self):
        ...
        for fence in (False, True):
            ...
            cache = df._vestigekv_fwd_grouped_kernel_stage1.device_caches[
            ...
            path = f"/tmp/vestigekv_fence_{int(fence)}.cubin"
