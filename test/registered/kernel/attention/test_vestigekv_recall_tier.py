...
class TestOmittedMass(CustomTestCase):
    ...
    def _parts(self, t, q, D):
        sc = t.scale
        qe = q.float()
        s_kept = (qe.to(torch.bfloat16) @ t.kept_rows.T).float() * sc
        max1 = t._thr_base(s_kept, s_kept.max(-1).values)
        ...
        fire = (score > (max1 - t.margin - t._ent_margin(s_kept))[:, None]) & (
            ent > t.thr_g
        )[:, None]
        return s_kept, max1, score, ~fire.any(0)
    ...
    def test_log_mass_matches_an_independent_sum_over_the_omitted_rows(self):
        ...
        s_kept, max1, score, om = self._parts(t, q, D)
    ...
    def test_sigma_is_a_blend_not_a_replacement(self):
        ...
        s_kept, max1, score, om = self._parts(t, q, D)
        fired = score[:, ~om]
        lse_att = torch.logsumexp(torch.cat([s_kept, fired], dim=-1), dim=-1)
