...
class VestigeKVMLABackend(AttentionBackend):
    ...
    _ingraph_dead = False
    # capture-reason counters (diagnostic; printed by VESTIGEKV_STATS when stats on)
    _cap_keymiss = 0
    _cap_fits = 0
    _cap_lru_hit = 0
    ...
    def _decode_prologue(self, forward_batch):
        ...
        # left an eager run with no VESTIGEKV_STATS at all, which is how a
    ...
    def _replay_scan(self, forward_batch, reqs, key) -> bool:
        ...
        # the ping-pong (VESTIGEKV_KEYMISS showed two shape classes evicting each
        ...
        if envs.SGLANG_DEBUG_VESTIGEKV_STATS.get() and self._cap_keymiss % 20 == 1:
            ...
            logging.getLogger(__name__).info(
                "VESTIGEKV_KEYMISS old=%s new=%s", self._scan_key_cur, key
            )
        return self._capture_scan(key, forward_batch, reqs)
    ...
    def _capture_scan(self, key, forward_batch, reqs) -> bool:
        ...
        if envs.SGLANG_DEBUG_VESTIGEKV_STATS.get():
            ...
            logging.getLogger(__name__).info(
                "VESTIGEKV_CAPTURE total=%.1fms pack=%.1f warm=%.1f rec=%.1f",
    ...
    def _capture_scan_timed(self, key, forward_batch, reqs) -> bool:
        ...
        if self._scan_cache is not None and len(self._scan_cache) > 1:
            ...
            # illegal address inside the s_kept cutlass bmm -- a use-after-free
    ...
    def _dump_stats(self):
        ...
        logging.getLogger(__name__).info(
            "VESTIGEKV_STATS steps=%d calls/step=%.2f layers=%d replay=%.0f%% build=%.1fms x%d cap=%.1fms x%d "
    ...
    def _install_finished_builds(self):
        ...
        for job in self._build_jobs:
            ...
            if envs.SGLANG_DEBUG_VESTIGEKV_STATS.get():
                ...
                logging.getLogger(__name__).info(
                    "VESTIGEKV_CAL lid=%s slot=%s seq=%s proxy=False async=True %s",
                    lid,
                    slot,
                    job["seq_len"],
                    stats,
    ...
    def _build_index(self, slot, lid, seq_len, st, proxy: bool):
        ...
        if envs.SGLANG_DEBUG_VESTIGEKV_STATS.get():
            ...
            logging.getLogger(__name__).info(
                "VESTIGEKV_BUILD_MS lid=%s slot=%s proxy=%s ms=%.1f",
                lid,
                slot,
                proxy,
                dt * 1e3,
    ...
    def _build_index_timed(self, slot, lid, seq_len, st, proxy: bool):
        ...
        if envs.SGLANG_DEBUG_VESTIGEKV_STATS.get():
            ...
            logging.getLogger(__name__).info(
                "VESTIGEKV_CAL lid=%s slot=%s seq=%s proxy=%s %s",
                lid,
                slot,
                seq_len,
                proxy,
    ...
    def _pack_csr(self, lid, slots, loc, real_bs, bs, kmax, dense=None):
        ...
        # (VESTIGEKV_STATS, S=64k bs=1). Masked-out entries scatter into the trash slot
