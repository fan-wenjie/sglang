"""VestigeKV for DSA models (GLM-5.3-Flash): the decode side of the split pair.

A copy of vestigekv_mla_backend.py taken 2026-09-22 so the DSA-specific work --
the DSA sibling's decode metadata, the fenced arm that attends the indexer's
selection, the salience ring, and the decode-kernel and scheduling
optimisations that follow -- can be done without touching the Kimi Linear
backend the paper's numbers come from. Registered as `vestigekv_dsa`; used as
`--prefill-attention-backend dsa --decode-attention-backend vestigekv_dsa`.
Behaviour at the time of the copy is identical to vestigekv_mla.
"""
...
def vestigekv_backend_of(backend):
    ...
    for _ in range(4):
        if isinstance(backend, HybridAttnBackend):
            ...
            break
    return backend if isinstance(backend, VestigeKVDSABackend) else None
