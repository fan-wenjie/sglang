...
def _vestigekv_row_loop(
    ...
    VESTIGEKV_CAP: tl.constexpr,
    VESTIGEKV_FW: tl.constexpr,
    VESTIGEKV_R2T: tl.constexpr,
    AFFINE: tl.constexpr,
):
    ...
    for start_n in tl.range(split_kv_start, split_kv_end, BLOCK_N):
        ...
        if AFFINE:
            ...
            kv_loc = (vestigekv_base + offs_n).to(tl.int64)
        else:
            if ROW_SRC == SRC_CSR:
                kv_loc = tl.load(
                ...
                # away, and vestigekv_last/vestigekv_loc are loop invariants hoisted above.
                live = offs_n < split_kv_end
                kept = tl.load(
                    vestigekv_kept_buf + vestigekv_slot * VESTIGEKV_CAP + offs_n,
                    mask=live & (offs_n < vestigekv_nk) & (not vestigekv_fenced),
                    other=0,
                )
                fired = tl.load(
                    vestigekv_fetch_buf
                    + vestigekv_slot * VESTIGEKV_FW
                    + (offs_n - vestigekv_nk),
                    mask=live & (offs_n >= vestigekv_nk) & (not vestigekv_fenced),
                    other=0,
                )
                rid = tl.load(
                    vestigekv_r2t + vestigekv_slot * VESTIGEKV_R2T + offs_n,
                    mask=live & vestigekv_fenced,
                    other=0,
                )
                tier = tl.where(offs_n < vestigekv_nk, kept, fired)
                fenced_row = tl.where(
                    offs_n == vestigekv_last, vestigekv_loc.to(rid.dtype), rid
                )
                kv_loc = tl.where(vestigekv_fenced, fenced_row, tier).to(tl.int64)
...
@triton.jit
def _vestigekv_fwd_grouped_kernel_stage1(
    Q,
    K_Buffer,
    V_Buffer,
    sm_scale_withk,
    kv_indptr,
    kv_indices,
    vestigekv_slots,  # [bs] int64 pool slot per lane
    vestigekv_kept_buf,  # [R1, CAP] int32 this layer's kept row ids
    vestigekv_kept_len,  # [R1] int32
    vestigekv_fetch_buf,  # [R1, FW] int32 this layer's fired row ids
    vestigekv_fetch_len,  # [R1] int32
    vestigekv_fetch_ovf,  # [R1] int32; nonzero means the lane attends its whole row set
    vestigekv_r2t,  # [R1 - 1, VESTIGEKV_R2T] int32 page table
    vestigekv_seq,  # [bs] int64 rows of a fenced lane
    vestigekv_loc_ptr,  # [bs] int64 this step's appended pool row
    Att_Out,
    Att_Lse,
    num_kv_splits,
    stride_qbs,
    ...
    if ROW_SRC == SRC_CSR:
        ...
        vestigekv_slot = 0
        vestigekv_nk = 0
        vestigekv_fenced = False
        vestigekv_loc = 0
        vestigekv_last = 0
        vestigekv_base = 0
    else:
        cur_batch_kv_start_idx = 0
        vestigekv_slot = tl.load(vestigekv_slots + cur_batch).to(tl.int64)
        vestigekv_nk = tl.load(vestigekv_kept_len + vestigekv_slot).to(tl.int64)
        vestigekv_fenced = False
        if FENCE:
            vestigekv_fenced = tl.load(vestigekv_fetch_ovf + vestigekv_slot) != 0
        vestigekv_seq = tl.load(vestigekv_seq + cur_batch).to(tl.int64)
        cur_batch_seq_len = tl.where(
            vestigekv_fenced,
            vestigekv_seq,
            vestigekv_nk + tl.load(vestigekv_fetch_len + vestigekv_slot).to(tl.int64),
        )
        ...
        vestigekv_loc = tl.load(vestigekv_loc_ptr + cur_batch)
        vestigekv_last = cur_batch_seq_len - 1
        # AFFINE builds read this; the others compile the load away with the arm.
        vestigekv_base = tl.load(vestigekv_r2t + vestigekv_slot * VESTIGEKV_R2T).to(
            tl.int64
        )
    ...
    if split_kv_end > split_kv_start:
        ...
        if AFFINE and vestigekv_fenced:
            acc, e_sum, e_max = _vestigekv_row_loop(
                acc,
                e_sum,
                e_max,
                split_kv_start,
                ...
                BLOCK_DPE=BLOCK_DPE,
                SCORE_MOD=SCORE_MOD,
                VESTIGEKV_CAP=VESTIGEKV_CAP,
                VESTIGEKV_FW=VESTIGEKV_FW,
                VESTIGEKV_R2T=VESTIGEKV_R2T,
                AFFINE=True,
            )
        else:
            acc, e_sum, e_max = _vestigekv_row_loop(
...
def decode_grouped_att_m_fwd(
    ...
    kv_indptr,
    kv_indices,
    vestigekv,  # VestigeKVRows or None; None keeps upstream's CSR behaviour
    num_kv_splits,
    max_kv_splits,
    sm_scale_withk,
    logit_cap,
    ...
    _vestigekv_fwd_grouped_kernel_stage1[grid](
        q,
        k_buffer,
        v_buffer,
        sm_scale_withk,
        ...
        Lk=Lk,
        Lv=Lv,
        VESTIGEKV_CAP=vestigekv.kept_buf.shape[1],
        VESTIGEKV_FW=vestigekv.fetch_buf.shape[1],
        VESTIGEKV_R2T=vestigekv.r2t.shape[1],
        ROW_SRC=SRC_TIERS if vestigekv.tiers else SRC_CSR,
        FENCE=vestigekv.fence,
        AFFINE=vestigekv.affine,
        HAS_MLA=has_mla,
        USE_PDL=use_pdl,
