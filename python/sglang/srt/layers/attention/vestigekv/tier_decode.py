...
class TierDecodeRouter(msgspec.Struct, dict=True):
    ...
    def __call__(
        ...
        **kw,
        ...
        vestigekv = self.rows.get(self.current_layer)
...
def _blend_omitted_mass(o, attn_lse, num_kv_splits, logm_buf, mu_buf, slots):
    ...
    if _BLEND_SEEN[0] == 0:
        ...
        logging.getLogger(__name__).info(
            "VESTIGEKV_BLEND active: omitted-mass blend is in the decode path"
        )
    ...
    if _BLEND_SEEN[0] % 500 == 1:
        ...
        logging.getLogger(__name__).info(
            "VESTIGEKV_BLEND sigma @call %d: min %.4f p50 %.4f max %.4f over %d (q, head)",
            _BLEND_SEEN[0],
            float(f.min()),
            float(f.median()),
            float(f.max()),
