...
def device_loading_context(module: torch.nn.Module, target_device: torch.device):
    ...
    if getattr(module, "afd_weights_absent", False):
        # This module was built without storage on purpose -- something else computes it, and
        # the weights were never allocated here. Staging would find a meta tensor and refuse it,
        # which is the right answer for a module that failed to load and the wrong one for a
        # module that was never meant to have anything. The flag is set by whoever built it that
        # way, so this cannot hide an accidental meta tensor: nothing sets it by accident.
        yield module
        return
