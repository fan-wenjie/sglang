...
class Stop:
    ...
    def __init__(
        self, upstream, *, min_batch: int = 1, max_wait_s: float = 0.002
    ) -> None:
    ...
    def _depart(self, layer: int, riding: list) -> None:
        ...
        with self.meter.timed("wire_out"):
            for (frame, sock), n in zip(riding, counts):
                send_frame(
                    sock, Frame.one(frame.request_id, layer, out[offset : offset + n])
                )
                offset += n
        if len(riding) > 1:
            self.merged += 1
...
class PoolUpstream:
    ...
    def feed_forward(
        self, request_id: int, layer: int, joined: torch.Tensor
    ) -> torch.Tensor:
    ...
    def round_trip(self, frame: Frame) -> Frame:
        ...
        return Frame(
            frame.request_id,
            frame.layer,
            tuple(self.client.collect_frame(handle, "cpu")),
            frame.op,
        )
...
def main() -> int:
    ...
    p.add_argument(
        "--port", type=int, default=9100, help="where the hosts reach this stop"
    )
    ...
    upstream = (
        EchoUpstream() if args.upstream == "none" else PoolUpstream(args.upstream)
    )
    stop = Stop(
        upstream, min_batch=args.min_batch, max_wait_s=args.max_wait_ms / 1000.0
    )
...
def serve(
    stop: Stop, host: str, port: int, ready: threading.Event | None = None
) -> None:
