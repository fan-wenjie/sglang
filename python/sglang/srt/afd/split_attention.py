...
That asymmetry is the whole point. Under Early-Q the query is projected from h_{l-N}, which is on
hand before layer l-N's feed-forward is issued, so the sweep -- the expensive half, linear in the
context length -- runs while the pool works. The join is one position per request.
...
expected to record the refusal, not swallow it. Refused today:
    ...
    MLA / DCP           forward_decode's own early returns, which merge on their own terms
...
class SweepResult:
    ...
    def __init__(self, layer_id: int, q: torch.Tensor, o: torch.Tensor, lse: torch.Tensor):
...
def split_refusal(backend, layer, forward_batch) -> str | None:
    ...
    if backend.use_mla:
        return "MLA decode takes its own path through forward_decode"
...
def _build_prefix(backend, forward_batch):
    ...
    total = int(seq_lens_sum) - bs if seq_lens_sum is not None else int(prefix_lens.sum())
    if total <= 0:
    ...
    which = torch.repeat_interleave(
        torch.arange(bs, dtype=torch.int32, device=device), prefix_lens, output_size=total
    )
...
def _run_partition(backend, layer, forward_batch, *, q, k, v, indptr, indices, splits,
    ...
    metadata.kv_indptr, metadata.kv_indices, metadata.num_kv_splits = indptr, indices, splits
    ...
    try:
        o = backend.forward_decode(q, k, v, layer, forward_batch, save_kv_cache=save_kv_cache)
    finally:
