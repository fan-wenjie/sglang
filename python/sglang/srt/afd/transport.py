...
Three consequences, and each is a thing that would be expensive to change later:
    ...
    completion is polled, not returned       a blocking send has nowhere to put "in flight", and
                                             in flight is the whole point of issuing ahead of the
                                             answer
...
class SocketTransport:
    ...
    def post_send(
        self, peer: socket.socket, region: Region, offset: int, length: int
    ) -> Transfer:
...
class BufferRing:
    ...
    def report(self) -> dict:
        with self._lock:
            return {
                "buffers": len(self._all),
                "in_flight": len(self._all) - len(self._free),
                "high_water": self._high_water,
                "bytes_each": self.nbytes,
            }
