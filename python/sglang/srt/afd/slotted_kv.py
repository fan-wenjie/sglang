...
comes from the caller's `length` rather than from here -- see `parked_sweeps`. This class answers
...
import torch
from sglang.srt.afd.slots import NoFreeSlot, SlotTable  # noqa: F401  (NoFreeSlot re-exported)
...
class SlottedKV:
    ...
    def __init__(self, *, slots: int, kv_heads: int, head_dim: int, v_head_dim: int,
                 max_context: int, device, dtype=torch.bfloat16) -> None:
        ...
        self._k: dict[int, torch.Tensor] = {}       # layer -> (slots, kv_heads, max_context, dim)
        self._v: dict[int, torch.Tensor] = {}
        self._held: dict[tuple[int, int], int] = {}  # (slot, layer) -> positions written
    ...
    def report(self) -> dict:
        with self._lock:
            held = sum(t.numel() * t.element_size()
                       for t in list(self._k.values()) + list(self._v.values()))
            layers = len(self._k)
        return {**self._table.report(bytes_held=held),
                "layers_allocated": layers, "max_context": self.max_context}


def sweep_slots(q: torch.Tensor, slots: torch.Tensor, lengths: torch.Tensor,
                buf_k: torch.Tensor, buf_v: torch.Tensor,
                *, scaling: float, kv_group: int) -> tuple:
    ...
    if reach == 0:
        ...
        return (torch.zeros(rows, heads, buf_v.shape[-1], device=q.device, dtype=acc),
