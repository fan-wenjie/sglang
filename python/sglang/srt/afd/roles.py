...
def install_host_routing(model, pool_addr: str, sweep_ahead, connect_timeout_s: float = 30.0):
    ...
    if arm is not None:
        ...
        return client, arm.install_on_host(model, client, sweep_ahead=sweep_ahead)
...
def run_pool(model, host: str, port: int, min_batch: int, max_wait_ms: int,
    ...
    departure = serve(
        ...
        span=(_arm_if_wanted().make_pool_runner(model, device=device)
              if _arm_if_wanted() is not None else None),
        host=host,
        port=port,
        min_batch=min_batch,
