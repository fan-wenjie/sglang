...
def _cuda_graph_on(args) -> bool:
    ...
    if getattr(args, "disable_cuda_graph", False):
        return False
    if getattr(args, "disable_decode_cuda_graph", False):
        return False
...
CHECKS = (
    ...
    (
        "tp_size",
        lambda a: getattr(a, "tp_size", 1) > 1,
        UNTESTED,
    ...
    (
        "pp_size",
        lambda a: getattr(a, "pp_size", 1) > 1,
        UNTESTED,
        "pipeline stages and a per-layer router have never been run together.",
    ),
    (
        "dp_attention",
        lambda a: bool(getattr(a, "enable_dp_attention", False)),
        UNTESTED,
    ...
    (
        ...
        lambda a: getattr(a, "speculative_algorithm", None) is not None,
        UNTESTED,
    ...
    (
        "lora",
        lambda a: bool(getattr(a, "enable_lora", False) or getattr(a, "lora_paths", None)),
        BROKEN,
    ...
    (
        "multimodal",
        lambda a: bool(getattr(a, "enable_multimodal", False)),
        UNTESTED,
    ...
    (
        "weight_update",
        lambda a: getattr(a, "rl_on_policy_target", None) is not None,
        BROKEN,
...
def _allowed() -> set:
    raw = os.environ.get("AFD_ALLOW_UNTESTED", "")
    ...
    for name, predicate, kind, why in CHECKS:
        ...
        if kind is BROKEN:
            ...
            logger.warning(
                ...
                "Nothing here knows whether the result is correct. %s", name, why,
            )
        else:
