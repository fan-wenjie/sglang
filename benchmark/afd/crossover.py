...
cached tokens costs less at a larger batch and A stays ahead longer.
    ...
    python benchmark/afd/crossover.py
"""
...
PROBES = {
    1: [16384, 65536, 131072],
    2: [16384, 65536, 131072],
    4: [16384, 65536, 131072],
    8: [16384, 49152, 98304],
    16: [8192, 24576, 49152],
    32: [4096, 12288, 24576],
    64: [2048, 8192, 16384],
...
def sweep_us(dev, batch, context):
    k = torch.randn(
        batch * context, KV_HEADS, HEAD_DIM, device=dev, dtype=torch.bfloat16
    )
    v = torch.randn(
        batch * context, KV_HEADS, HEAD_DIM, device=dev, dtype=torch.bfloat16
    )
    ...
    # dropped by rebinding rather than by `del`: the lambda above closes over these names in
    # this same scope, and deleting a name a closure refers to is an F821 under the repository's
    # ruff selection even though `timed` has already called it. Rebinding frees the tensors just
    # the same -- the closure is dead by here either way.
    k = v = logits = lse = None
...
def main():
    ...
    rows = []
    print(
        f"  {'batch':>6} {'ffn':>8} {'proj':>7} {'sweep slope':>14} "
        f"{'THRESHOLD ctx':>14} {'tokens held':>13} {'cache there':>12}",
        flush=True,
    )
    ...
    for batch in wanted:
        ...
        F = timed(
            lambda: (torch.nn.functional.silu(x @ gate.t()) * (x @ up.t())) @ down.t()
        )
        ...
        print(
            f"  {batch:6d} {F:7.0f}u {P:6.0f}u {slope*1e3:10.4f}u/kT "
            f"{threshold:14,.0f} {threshold*batch:13,.0f} {cache_gb:10.1f} GB",
            flush=True,
        )

    print(
        f"\n  the rule: switch when the sweep reaches the feed-forward. Below it the "
        f"feed-forward should",
        flush=True,
    )
    print(
        f"  be the remote piece; above it the sweep should. The threshold in CONTEXT falls as "
        f"the batch",
        flush=True,
    )
    print(
        f"  grows and the tokens held RISES, because a bigger batch sweeps the same bytes more "
        f"cheaply --",
        flush=True,
    )
    print(
        f"  so a scheduler cannot carry one context number; it has to carry this condition.",
        flush=True,
    )
    json.dump(
        rows, open("/home/user/experiment/sglang/afd_crossover.json", "w"), indent=2
    )
    return 0
