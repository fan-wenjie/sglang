...
def main():
            ...
            rows.append(
                {
                    "tokens": tokens,
                    "round_trip_ms": ms,
                    "us_per_token": ms * 1e3 / tokens,
                }
            )
            print(
                f"    {tokens:4d} token(s): {ms:7.3f} ms round trip = "
                f"{ms * 1e3 / tokens:8.1f} us/token",
                flush=True,
            )
    finally:
        client.close()
    ...
    print(
        f"  so one pool read shared across callers is worth up to {base / best:.1f}x",
        flush=True,
    )
    json.dump(
        {"hidden": HIDDEN, "rows": rows, "amortisation": base / best},
        open(sys.argv[2], "w"),
        indent=2,
    )
    return 0
