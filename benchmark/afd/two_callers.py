...
def main() -> int:
    ...
    print(
        f"  {'tokens':>7} {'callers':>8} {'median ms':>10} {'calls/s':>9} {'tokens/s':>10} "
        f"{'vs 1 caller':>12}"
    )
    for tokens in args.tokens:
        ...
        for callers in args.callers:
            ...
            print(
                f"  {tokens:>7} {got['callers']:>8} {got['round_trip_ms']:>10.2f} "
    ...
    print(
        "  the first one's weight read shows tokens/s near 2x; one that waits its turn shows it"
    )
    print(
        "  flat. At 4 tokens a call the read is not what binds the pool, so the comparison only"
    )
    print(
        "  means something where the read dominates -- which is what the widths are for."
    )
    return 0
