...
def build(mib: int, width: int, dtype=torch.bfloat16):
    ...
    weights = [torch.randn(width, width, dtype=dtype, device="cuda") for _ in range(count)]
...
def main() -> int:
    ...
    print(f"  {args.rows} row(s) x {args.width}, bfloat16, {args.iters} iterations a point")
    print(f"  {'weights':>9} {'kernels':>8} {'per call':>10} {'GB/s':>8} {'graph':>10} "
    ...
    for mib in args.mib:
        ...
        print(f"  {got / 1024 ** 2:>8.0f}M {len(weights):>8} {plain * 1e3:>9.3f}ms {gbs:>8.0f} "
    ...
    print("  than reads. A pool whose floor is reads gains nothing from capturing a graph.")
    return 0
